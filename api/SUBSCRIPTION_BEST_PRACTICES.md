...
```go
import "github.com/0x3639/znn-sdk-go/api"
...
func subscribeToMomentums() error {
    ...
    momentumSub, err := client.SubscriberApi.ToMomentums()
    if err != nil {
        return err
    }
    ...
    accountSub, err := client.SubscriberApi.ToAllAccountBlocks()
    if err != nil {
        return err
    }
...
func watchFrontierMomentum() error {
    sub, err := client.SubscriberApi.ToMomentums()
    if err != nil {
        return err
    }
...
```go
sub, _ := client.SubscriberApi.ToMomentums()
...
func monitorNetwork() error {
    ...
    subs := []struct{
        ...
        {"momentums", client.SubscriberApi.ToMomentums()},
        {"blocks", client.SubscriberApi.ToAllAccountBlocks()},
        {"unconfirmed", client.SubscriberApi.ToAllUnconfirmedAccountBlocks()},
    }
...
func (s *MonitorService) Start() error {
    ...
    // Subscribe to events
    sub, err := s.client.SubscriberApi.ToMomentums()
    if err != nil {
        return err
    }
    s.manager.Add(sub)
...
func TestMultipleSubscriptions(t *testing.T) {
    tests := []struct {
        ...
        {"momentums", func(c *rpc_client.RpcClient) (*server.ClientSubscription, error) {
            return c.SubscriberApi.ToMomentums()
        }},
        {"blocks", func(c *rpc_client.RpcClient) (*server.ClientSubscription, error) {
            return c.SubscriberApi.ToAllAccountBlocks()
        }},
    }
...
go func() {
    sub, _ := client.SubscriberApi.ToAllAccountBlocks()
    manager.Add(sub)  // Thread-safe
}()
...
for _, account := range accounts {
    sub, _ := client.SubscriberApi.ToAccountBlocksByAddress(account)
    manager.Add(sub)
}
