...
func Example_subscribeMomentums() {
	...
	// Monitor momentums for 30 seconds
...
func Example_paymentGateway() {
	...
	sub, blockChan, err := client.SubscriberApi.ToUnreceivedAccountBlocksByAddress(gatewayAddress)
	if err != nil {
		log.Fatal(err)
	}
	defer sub.Unsubscribe()
...
func Example_multipleSubscriptions() {
	...
	// Subscribe to momentums
	momentumSub, momentumChan, err := client.SubscriberApi.ToMomentums()
	if err != nil {
		log.Fatal(err)
	}
	...
	accountSub, accountChan, err := client.SubscriberApi.ToAccountBlocksByAddress(myAddress)
	if err != nil {
		log.Fatal(err)
	}
